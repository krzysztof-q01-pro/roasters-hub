    ...
    <>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h1 className="font-headline text-4xl font-bold text-on-background mb-2">Cafes</h1>
        ...
        <div className="bg-surface-container-lowest rounded-xl overflow-hidden editorial-shadow">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {cafes.length === 0 && (
                <tr><td className="px-5 py-8 text-center text-on-surface-variant" colSpan={6}>No cafes match the filter.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-3 mt-6">
            {page > 1 && (
              <Link href={`/admin/cafes${buildQuery({ page: page - 1 })}`} className="px-4 py-2 rounded-lg bg-surface-container-low font-bold hover:bg-surface-container">← Prev</Link>
            )}
            <span className="text-sm text-on-surface-variant">Page {page} / {totalPages}</span>
            {page < totalPages && (
              <Link href={`/admin/cafes${buildQuery({ page: page + 1 })}`} className="px-4 py-2 rounded-lg bg-surface-container-low font-bold hover:bg-surface-container">Next →</Link>
            )}
          </div>
        )}
      </main>
